...
class Review(models.Model):
    ...
    triggered_by = models.CharField(
        max_length=20, choices=Trigger.choices, default=Trigger.OPENED
    )
    status = models.CharField(
        max_length=20, choices=Status.choices, default=Status.QUEUED
    )
    latency_ms = models.IntegerField(null=True, blank=True, help_text="LLM processing time in ms")
    token_cost = models.IntegerField(null=True, blank=True, help_text="Total tokens used across all LLM calls")
